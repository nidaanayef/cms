...
@section("title","اضافة مقال جديد")

@section("content")

<form class="w-50" method="POST" action="{{route('article.store')}}">
@csrf
                  <!--add article title-->
                  <div class="form-group">
										<label for="title">عنوان المقالة</label>
										<input autofocus value="{{ old('title') }}" id="title" class="form-control" type="text" name="title" placeholder="عنوان المقالة">
                  </div>
                  <!--add article sub title-->
                  <div class="form-group">
										<label for="sub_title">العنوان الفرعي للمقالة</label>
										<input value="{{ old('sub_title') }}" id="sub_title" class="form-control" type="text" name="sub_title" placeholder="العنوان الفرعي للمقالة">
									</div>
							
                  <!--add photo to article-->
                  <div class="form-group">
											<!--select class="form-control"  id="photos_id" name="photos_id">
                        <option value="">اضافة صورة المقال</option>
                        @foreach($photos as $photo)
													<option {{ old("photos_id")==$photo->id?"selected":"" }} value="{{ $photo->id }}">{{ $photo->file }}</option>
                        @endforeach
											</select-->
                  ...
                  <!--add type to article-->
									<div class="form-group">
										<label for="types_id"> نوع للمقال</label>
                  ...
                  <!--add category to article-->
									<div class="form-group">
										<label for="categories_id"> تصنيف للمقال</label>
             ...
             <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <input type='text' class="form-control " name="date" placeholder="تاريخ ووقت كتابة المقالة" id='date' value="{{old('date')}}" />
                 ...
                 <div class="form-group">
										<label for="summary">ملخص المقالة</label>
												<textarea maxlength="191" id="summary" class="form-control" name="summary" placeholder="ملخص المقالة">{{ old('summary') }}</textarea>
											</div>
